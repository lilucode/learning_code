 ...
 * 交易描述：默认服务初次化钩子场景 <br/>
 *
 * 创建时间：2018-11-22 03:19 <br/>
 *
 * 修改时间：2018-11-22 05:33 <br/>
 *
 * @author  <br/>
 * @version 1.0 <br/>
 *
...
public class T__Initializer__ extends JScript {
    ...
    protected class Step1 implements INode {
        ...
        private class Node4 implements INode {
            ...
            @Override
            public INode execute() {
                log(LogLevel.INFO, "Step1_Node4 默认逻辑错误委托");
        ...
        private class Node5 implements INode {
            ...
            @Override
            public INode execute() {
                log(LogLevel.INFO, "Step1_Node5 异常结束");
                return EndNode.EXCEPTION_END;
            }    
        }
        ...
        private class Node6 implements INode {
            ...
            @Override
            public INode execute() {
                log(LogLevel.INFO, "Step1_Node6 开始");
                return new Node4();
            }    
        }
        ...
        private class Node7 implements INode {
            ...
            @Override
            public INode execute() {
                log(LogLevel.INFO, "Step1_Node7 取全局错误到容器");
                setExceptionHandler(null);
                startTime = System.currentTimeMillis();
                try {
                    ...
                    if (result == null) {
                    	...
                    	gatherStat("Step1_Node7", "取全局错误到容器", startTime, "技术组件返回值不能为空");
                	...
                	gatherStat("Step1_Node7", "取全局错误到容器", status, startTime);
                    switch (status) {
                    case 1:
                        return new Node5();
                    default:
                	...
                	gatherStat("Step1_Node7", "取全局错误到容器", startTime, ExceptionUtils.toDetailString(e));
                    setGlobalError("E", "ACMP0E001", e.toString());
                    log(LogLevel.ERROR, e);
                    INode exceptionHandler = getExceptionHandler(new Node5());
        ...
        private class Node8 implements INode {
            ...
            @Override
            public INode execute() {
                log(LogLevel.INFO, "Step1_Node8 参数内存化");
                startTime = System.currentTimeMillis();
                try {
                    String _arg0_ = "1";
                    ...
                    if (result == null) {
                        log(LogLevel.ERROR, "技术组件返回值不能为空");
                    	gatherStat("Step1_Node8", "参数内存化", startTime, "技术组件返回值不能为空");
                        return getExceptionHandler(new Node5());
                    }
                	...
                	gatherStat("Step1_Node8", "参数内存化", status, startTime);
                    switch (status) {
                    case 1:
                        return new Node3();
                    default:
                	...
                	gatherStat("Step1_Node8", "参数内存化", startTime, ExceptionUtils.toDetailString(e));
                    setGlobalError("E", "ACMP0E001", e.toString());
                    log(LogLevel.ERROR, e);
                    INode exceptionHandler = getExceptionHandler(new Node5());
                    if (exceptionHandler == null) {
