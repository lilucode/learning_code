 ...
 * 交易描述：将缓存写入redis <br/>
 *
 * 创建时间：2018-10-31 09:16 <br/>
 *
 * 修改时间：2018-11-02 10:40 <br/>
 *
 * @author  <br/>
 * @version 1.0 <br/>
 *
 */
public class TINITCACHE extends JScript {
    protected INode startStep;
    protected long startTime;
     
    public TINITCACHE(JavaDict __REQ__, JavaDict __RSP__) {
    ...
    public synchronized INode execute() {
        ...
        if (startStep == null) {
            ...
            startStep = new Step1();
            __REQ__.setItem("__TRADENAME__", "将缓存写入redis");
            log(LogLevel.INFO, "log start:" + uuid);
            log(LogLevel.INFO, "开始交易  INITCACHE:将缓存写入redis");
            printTrace();
        ...
        if (step == EndNode.SUSPEND_END) {
            ...
            log(LogLevel.INFO, "结束交易 INITCACHE");
    ...
    protected class Step1 implements INode {
        ...
        public INode execute() {
            ...
            gatherStat("Step1", "逻辑处理", 1, startTime);
        ...
        private class Node4 implements INode {
            ...
            @Override
            public INode execute() {
                log(LogLevel.INFO, "Step1_Node4 参数内存化");
                startTime = System.currentTimeMillis();
                try {
                    String _arg0_ = "1";
                    ...
                    if (result == null) {
                    	...
                    	gatherStat("Step1_Node4", "参数内存化", startTime, "技术组件返回值不能为空");
                	...
                	gatherStat("Step1_Node4", "参数内存化", status, startTime);
                    switch (status) {
                    case 1:
                        return new Node5();
                    default:
                	...
                	gatherStat("Step1_Node4", "参数内存化", startTime, ExceptionUtils.toDetailString(e));
        ...
        private class Node5 implements INode {
            ...
            @Override
            public INode execute() {
                log(LogLevel.INFO, "Step1_Node5 正常结束");
        ...
        private class Node6 implements INode {
            ...
            @Override
            public INode execute() {
                log(LogLevel.INFO, "Step1_Node6 取全局错误到容器");
                setExceptionHandler(null);
                startTime = System.currentTimeMillis();
                try {
                    ...
                    if (result == null) {
                        log(LogLevel.ERROR, "技术组件返回值不能为空");
                    	gatherStat("Step1_Node6", "取全局错误到容器", startTime, "技术组件返回值不能为空");
                        return getExceptionHandler(new Node7());
                    }
                	...
                	gatherStat("Step1_Node6", "取全局错误到容器", status, startTime);
                    switch (status) {
                    case 1:
                        return new Node7();
                    default:
                	...
                	gatherStat("Step1_Node6", "取全局错误到容器", startTime, ExceptionUtils.toDetailString(e));
                    setGlobalError("E", "ACMP0E001", e.toString());
                    log(LogLevel.ERROR, e);
                    INode exceptionHandler = getExceptionHandler(new Node7());
                    if (exceptionHandler == null) {
        ...
        private class Node7 implements INode {
            ...
            @Override
            public INode execute() {
                log(LogLevel.INFO, "Step1_Node7 异常结束");
                return EndNode.EXCEPTION_END;
            }    
        }
